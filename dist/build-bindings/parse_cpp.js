...
export function getConstraintFunctions() {
    ...
    const functions = tsq.queryConstraintFunctions(src_string).map(item => (Object.assign(Object.assign({}, item), { params_list: item.params_list.map(param => (Object.assign(Object.assign({}, param), { is_enum: exported_enums.map(e => e.enum_name).includes(param.type) }))), definition_params: params_to_definition_string(item.params_list), call_params: params_to_call_string(item.params_list), fname_lower: camelToSnakeCase(item.fname) })));
    ...
    return functions.map(fn => {
        ...
        return Object.assign(Object.assign({}, fn), { fname_lower, 
            // data for typescript definitions
            constraint_name: fn.fname.replace('addConstraint', '') + specificator_str.toUpperCase(), constraint_type: fname_lower.replace('add_constraint_', '') });
    });
}
...
export function params_to_definition_string(params) {
    const arr = [];
...
export function params_to_call_string(params) {
    const arr = [];
    for (const param of params) {
        if (param.type === 'double' && param.identifier.startsWith('*')) {
            const id = double_pointer_param_to_param_i(param.identifier);
